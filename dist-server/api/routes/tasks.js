...
async function getTasks() {
    try {
        const tasks = await googleSheets_1.googleSheetsAPI.readSheet('tasks');
...
async function createTask(taskData) {
    try {
        ...
        await googleSheets_1.googleSheetsAPI.appendToSheet('tasks', values);
...
async function updateTask(id, taskData) {
    try {
        ...
        await googleSheets_1.googleSheetsAPI.updateSheet('tasks', `A${task._rowNumber}:N${task._rowNumber}`, [values]);
...
async function deleteTask(id) {
    try {
        ...
        await googleSheets_1.googleSheetsAPI.updateSheet('tasks', `A${task._rowNumber}:N${task._rowNumber}`, [emptyValues]);
        return { success: true };
    }
    catch (error) {
